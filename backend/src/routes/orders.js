...
// Upload daily orders (duplicate endpoint for compatibility)
router.post('/orders/daily-uploads', upload.single('file'), async (req, res) => {
  if (!req.file) {
    return res.status(400).json({ error: 'No file uploaded. Please upload a file with the field name "file".' });
  }
  const { company_id, store_id, order_date } = req.body;
  const filePath = req.file.path;
